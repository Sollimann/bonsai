...
| [test_behavior.py](test_behavior.py) | All 14 factories exported and callable; each builds with expected repr (including `memory=False` composites); validation guards (empty `While`/`WhileAll` body, NaN `Wait`); empty composites + neg/inf/int `Wait` pass through; subtree reuse; kwargs on `If`; tuple accepted / generator rejected for children; identity-based equality per variant; ported Rust `behavior_tests.rs` cases (immediate termination, wait timing, select short-circuit, if branches, invert, always-succeed, when-all, while-loop). |
| [test_memory.py](test_memory.py) | The `memory` flag on `Sequence` / `Select`: memoryless (`memory=False`) short-circuit + re-evaluation semantics ported from the Rust tests, the default (`memory=True`) resume behavior, and repr (`Sequence(N, memory=False)`). |
...
| [test_module.py](test_module.py) | `__version__ == "0.13.0"`; module docstring present; `__all__` lists exactly the 19 expected names; all names accessible; `RUNNING` is `(Status.Running, 0.0)`; `.pyi` stub ships with the wheel. |
